...
class VisitorLogApp extends JFrame implements LoginPanel.LoginHandler {
  ...
  private final VisitorTableModel visitorTableModel = new VisitorTableModel(guardDirectory.getGuards(), adminDirectory.getAdmins());
  private final ImageIcon ubLogoIcon = loadLogoIcon();
  ...
  private void buildDashboards() {
    guardDashboard = new GuardDashboardPanel(guardDirectory, adminDirectory, visitorTableModel);
    adminDashboard = new AdminDashboardPanel(guardDirectory, adminDirectory, visitorTableModel, this::refreshDashboards);
  ...
  private JPanel wrapWithNav(JPanel content, String title, Runnable logoutAction) {
    ...
    if (ubLogoIcon != null) {
      leftGroup.add(new JLabel(ubLogoIcon));
    }
