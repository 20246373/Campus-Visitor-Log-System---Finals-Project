...
class AdminDashboardPanel extends JPanel {
  ...
  AdminDashboardPanel(GuardDirectory guardDirectory,
    ...
    logPanel = new VisitorLogPanel(guardDirectory.getGuards(), adminDirectory.getAdmins(), true);
    GuardManagementPanel managementPanel = new GuardManagementPanel(guardDirectory, () -> {
      logPanel.refreshPersonnel(guardDirectory.getGuards(), adminDirectory.getAdmins());
      onGuardRosterChanged.run();
    });
    ...
    JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, logPanel, managementPanel);
    ...
    exportJson.setToolTipText("Export the entire visitor log to JSON file in working directory.");
    ...
    exportJson.addActionListener(e -> {
      logPanel.exportJson("visitor_logs.json");
      JOptionPane.showMessageDialog(this, "Exported visitor_logs.json", "Export", JOptionPane.INFORMATION_MESSAGE);
    });
    exportTxt.addActionListener(e -> {
      logPanel.exportReadableTxt("visitor_logs_readable.txt");
      JOptionPane.showMessageDialog(this, "Exported visitor_logs_readable.txt", "Export", JOptionPane.INFORMATION_MESSAGE);
    });
